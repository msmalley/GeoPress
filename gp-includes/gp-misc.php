...
/**
 *
 * Added in 0.0.4 by Mark Smalley
 * Sinmple de-bugging function...
 * 
 * @param <type> $arr
 * @param <type> $exit
 */
function gp_printr($arr, $exit = true){
    echo '<pre>';
    if(is_array($arr)){
...
/**
 * Gets data from the database and returns it as an array...
 * Added in 0.0.4 by Mark Smalley
 *
 */
function gp_get_data(
	...
	$meta_value = maybe_unserialize( $meta_value );
        if((!empty($meta_key))&&(!empty($meta_value))){
            if($table=='gp_checkins'){
                $meta_array = $gpdb->get_results("SELECT * FROM `$table` WHERE announcementtype !='whisper' AND $meta_key = $meta_value ORDER BY checkin_time DESC LIMIT $limit",ARRAY_A);
                //echo "SELECT * FROM `$table` WHERE announcementtype !='whisper' AND $meta_key = $meta_value ORDER BY checkin_time DESC LIMIT $limit"; exit;
            }else{
                $meta_array = $gpdb->get_results("SELECT * FROM `$table` WHERE private !=1 AND $meta_key = $meta_value ORDER BY created DESC LIMIT $limit",ARRAY_A);
            }
        }elseif(!empty($meta_value)){
            // Need to build these queries...
        }elseif(!empty($meta_key)){
            // Need to build these queries...
        }elseif($object_id>0){
            // Need to build these queries...
        }else{
            $meta_array = $gpdb->get_results("SELECT * FROM `$table` WHERE private !=1 ORDER BY id DESC LIMIT $limit",ARRAY_A);
        }
