<?php
class GP_Checkin extends GP_Thing {
	
	var $table_basename = 'checkins';
	var $field_names = array( 'id', 'mapid', 'placeid', 'latlng', 'mode', 'slug', 'path', 'announcement', 'announcementtype', 'parent_checkin_id', 'checkin_time' );
	...
	function path_to_root() {
		$path = array();
		if ( $this->parent_checkin_id ) {
			$parent_map = $this->get( $this->parent_checkin_id );
