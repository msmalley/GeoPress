 ...
 *
 * @param string $url Optional. The default is the GeoPress URL
 */
 ...
 * 
 * Example: gp_url_join( '/map', array( 'wp', 'dev) ) -> '/map/wp/dev'
 * 
...
/**
 * Builds a URL relative to the GeoPress' domain root
 * 
...
/**
 * Converts an absolute URL to the corresponding SSL URL if the GeoPress
 * settings allow SSL
 */
...
/**
 * Constructs URL for a map and locale.
 * /<map-path>/<locale>/<path>/<page>
 */
function gp_url_map_locale( $map_or_path, $locale, $path = '', $query = null ) {
	return gp_url_map( $map_or_path, array( $locale, $path ), $query );
}

function gp_url_img( $file ) {
	return gp_url_base( array( 'gp-admin/img', $file ) );
}
...
function gp_url_current() {
	$default_port = is_ssl()? 443 : 80;
        $default_port_mac = is_ssl()? 443 : 8888;
	$host = gp_array_get( $_SERVER, 'HTTP_HOST' );
	if ( gp_array_get( $_SERVER, 'SERVER_PORT', $default_port ) != ($default_port || $default_port_mac) ) $host .= ':' . gp_array_get( $_SERVER, 'SERVER_PORT' );
