...
require_once( GP_PATH . GP_INC . '/gp-init.php' );
...
if ( !defined( 'GP_LOCALES_PATH' ) ) {
	define( 'GP_LOCALES_PATH', GP_PATH . 'gp-includes/langs/' );
}

if ( !defined( 'GP_LANG_PATH' ) ) {
	define( 'GP_LANG_PATH', GP_PATH . 'gp-lang/' );
}
...
if ( in_array( GP_DATABASE_CLASS, array( 'BPDB', 'BPDB_Multi' ) ) ) {
	...
	if ( !defined( 'BPDB__ERROR_STRING' ) ) {
		define( 'BPDB__ERROR_STRING', 'ERROR: GeoPress database error - "%s" for query "%s" via caller "%s"' );
	}
	...
	if ( !defined( 'BPDB__DB_VERSION_ERROR' ) ) {
		define( 'BPDB__DB_VERSION_ERROR', 'ERROR: GeoPress requires MySQL 4.0.0 or higher' );
	}
	if ( !defined( 'BPDB__PHP_EXTENSION_MISSING' ) ) {
		define( 'BPDB__PHP_EXTENSION_MISSING', 'ERROR: GeoPress requires The MySQL PHP extension' );
	}
}
...
$gpdb->table_names = array('maps', 'users', 'places', 'checkins', 'usermeta', 'meta', 'permissions', );
...
if ( !defined( 'GP_TMPL_PATH' ) )
	define( 'GP_TMPL_PATH', GP_PATH . 'gp-content/themes/default/' );
	
if ( !defined( 'GP_ADMIN_PATH' ) )
	define( 'GP_ADMIN_PATH', GP_PATH . 'gp-admin/' );

require_once( GP_PATH . GP_INC . 'gp-meta.php' );
require_once( GP_PATH . GP_INC . 'gp-misc.php' );
require_once( GP_PATH . GP_INC . 'gp-permalinks.php' );
require_once( GP_PATH . GP_INC . 'gp-strings.php' );

require_once( GP_PATH . GP_INC . 'gp-templates.php' );
require_once( GP_PATH . GP_INC . 'gp-maps.php' );
require_once( GP_PATH . GP_INC . 'gp-places.php' );
require_once( GP_PATH . GP_INC . 'gp-checkins.php' );

require_once( GP_PATH . GP_INC . 'gp-geoforms.php' );
...
require_once( BACKPRESS_PATH . 'class.bp-options.php' );
...
require_once( GP_PATH . GP_INC . 'gp-assets.php' );
require_once( GP_PATH . GP_INC . 'gp-filters.php' );
require_once( GP_PATH . GP_INC . 'gp-langs.php' );

require_once( GP_LOCALES_PATH . 'mo.php' );
require_once( GP_LOCALES_PATH . 'po.php' );
...
if ( !class_exists( 'WP_Auth' ) ) {
	...
	$cookies['auth'][] = array(
		...
		'path' => gp_url_path(),
		'name' => gp_const_get( 'GP_AUTH_COOKIE', 'geopress_auth' ),
	);
	$cookies['secure_auth'][] = array(
		'domain' => GP_COOKIE_DOMAIN,
		'path' => gp_url_path(),
		'name' => gp_const_get( 'GP_SECURE_AUTH_COOKIE', 'geopress_sec_auth' ),
		'secure' => 'true',
	);
	...
	$cookies['logged_in'][] = array(
		'domain' => GP_COOKIE_DOMAIN,
		'path' => gp_url_path(),
		'name' => gp_const_get( 'GP_LOGGED_IN_COOKIE', 'geopress_logged_in' ),
	);
	$wp_auth_object = new WP_Auth( $gpdb, $wp_users_object, $cookies );
	unset( $cookies );
}

require_once( GP_PATH . GP_INC . 'gp-validation.php' );

require_once GP_PATH . GP_INC . 'gp-components.php';
foreach( glob( GP_PATH . GP_INC . 'components/*.php' ) as $thing_file ) {
	require_once $thing_file;
}

require_once( GP_PATH . GP_INC . 'gp-redirect.php' );
require_once( GP_PATH . GP_INC . 'gp-redirects.php' );
foreach( glob( GP_PATH . GP_INC . 'redirects/*.php' ) as $route_file ) {
...
require_once( GP_PATH . GP_INC . 'gp-plugins.php' );
